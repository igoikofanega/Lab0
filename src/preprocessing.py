...
def remove_missing(values: List[Any]) -> List[Any]:
    """Remove missing values: None, '', float('nan')"""
    return [v for v in values if v is not None and v != '' and str(v).lower() != 'nan']

def fill_missing(values: List[Any], fill_value: Any = 0) -> List[Any]:
    """Replace missing values with fill_value"""
    return [fill_value if v is None or v == '' or str(v).lower() == 'nan' else v for v in values]
...
def normalize(values: List[float], new_min: float = 0.0, new_max: float = 1.0) -> List[float]:
    ...
    return [new_min + (v - v_min) * (new_max - new_min) / (v_max - v_min) for v in values]
